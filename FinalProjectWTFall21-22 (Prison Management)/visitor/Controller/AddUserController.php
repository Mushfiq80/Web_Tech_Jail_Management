...
elseif(isset($_POST["btn_login"])){
  ...
  if(!$hasError){
   ...
   $err_db = "User ID and password invalid";
   //header("Location: login.php");
...
function insertUser($id,$username,$password,$gender,$email,$address,$usertype){
    $query = "insert into visitor values ($id,'$username','$password','$gender','$email','$address','$usertype');";
...
function authenticateUser($uid,$password){
  // var_dump($uid);
   //var_dump($password);
   $query = "select * from visitor where id=$uid and password='$password'";
   $rs = get($query);
   //var_dump($rs);
   if(count($rs) > 0){
      return true;
...
function typecheck($id){
   $query = "select usertype from visitor where id='$id'";
   $rs = get($query);
   return $rs;	
}
